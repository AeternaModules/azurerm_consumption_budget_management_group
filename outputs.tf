...
output "consumption_budget_management_groups_filter" {
  ...
  value       = { for k, v in azurerm_consumption_budget_management_group.consumption_budget_management_groups : k => one(v.filter) if v.filter != null && length(v.filter) > 0 }
}
...
output "consumption_budget_management_groups_time_period" {
  ...
  value       = { for k, v in azurerm_consumption_budget_management_group.consumption_budget_management_groups : k => one(v.time_period) if v.time_period != null && length(v.time_period) > 0 }
}
